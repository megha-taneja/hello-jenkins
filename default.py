...
#                    self._display.display("Task name and hostname %s and %s" % (taskname, result._host.get_name()))
                    ...
                    if (taskname in self.vertical_failed_tasks.keys()):
                        self.vertical_failed_tasks[taskname] = self.vertical_failed_tasks[taskname] + [result._host.get_name()]
                    else:
...
#        self._display.display("stats : %s" % (stats))
...
#             self._display.display("Count value %s:" %(count_failed))
             if t['unreachable'] == 1:
                 nodes_unreachable.append(h)
                 count_unreachable = count_unreachable + 1
        self._display.banner ("Playbook Executed: %s" % (self.playbook_name))
        ...
        if len(nodes_unreachable) > 0:
            ...
            self._display.display("%s" % (colorize(u'UNREACHABLE', ', '.join(nodes_unreachable), C.COLOR_UNREACHABLE)))
        else:
